...
calibration_table %>%
  ...
  modeltime_forecast(h="1 month", actual_data = brent_prices) %>%
  plot_modeltime_forecast(.interactive = T, .plotly_slider = T)

#zmiana time units 

#probujemy dalej i uzywamy Varmy razem z Dolar index

dolar_index = tq_get("DX-Y.NYB")
dolar_index %>% plot_time_series(date, open, .smooth = T,
                                 .smooth_size = 0.5,
                                 .smooth_alpha = 0.7,
                                 .smooth_degree = 2,
                                 .smooth_period = "1 year",
